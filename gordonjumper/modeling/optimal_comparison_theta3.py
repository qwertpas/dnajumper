...
def optimize_tsa(params, guess=None, N=40, max_attempts=15, verbose=False):
    ...
    # Compute required theta_margin based on geometry
    min_tm = compute_min_theta_margin(L_min, stroke + y_off_max)
    base_tm = min(0.98, min_tm + 0.02)
    
    # Attempt configurations: (N, max_iter, tol, theta_margin)
    configs = [
        (15, 500, 1e-4, base_tm),
        (15, 500, 1e-4, min(0.98, base_tm + 0.02)),
        (20, 600, 1e-4, base_tm),
        (20, 600, 1e-4, min(0.98, base_tm + 0.02)),
        (25, 700, 1e-5, base_tm),
        (30, 800, 1e-5, base_tm),
        (35, 1000, 1e-6, base_tm),
        (40, 1200, 1e-6, base_tm),
        (40, 1500, 1e-6, min(0.98, base_tm + 0.02)),
        (50, 1500, 1e-6, base_tm),
        (50, 2000, 1e-6, min(0.98, base_tm + 0.02)),
        (60, 2500, 1e-6, base_tm),
    ]
    ...
    for idx in range(min(max_attempts, len(configs))):
        N_try, max_iter, tol, theta_margin = configs[idx]

        # Vary guesses systematically
        L_try = L_min + (L_max - L_min) * (0.7 + 0.3 * ((idx % 4) / 3))
    ...
    for i in range(n_tests):
        ...
        for i in range(n_tests):
            ...
            try:
                ...
                # Verify solution numerically (unless this is redundant/expensive)
                verification = verify_solution(params, result, verbose=False)
                if verification['valid']:
                    ...
                    print(f"-> L={result['L']*1e3:.1f}mm, r={result['r']*1e3:.2f}mm, "
                          f"ydot={result['ydot_final']:.3f} m/s INVALID ✗")
                    results.append({**result, 'verified': False})
            except RuntimeError:
                print("-> FAILED ✗")
                results.append(None)
        
        # Summary
        successes = [r for r in results if r is not None and r.get('verified', False)]
        print("-" * 70)
